...
def neuralNetwork(input_layer_size, hidden_layer_size, out_put_layer):
    ...
    initial_nn_params = np.vstack(
        (initial_Theta1.reshape(-1, 1), initial_Theta2.reshape(-1, 1)))  # 展开theta
    ...
    np.savetxt("code/BP/predict.csv", res, delimiter=',')
...
def nnGradient(nn_params, input_layer_size, hidden_layer_size, num_labels, X, y, Lambda):
    ...
    Theta1 = nn_params[0:hidden_layer_size*(input_layer_size+1)].reshape(
        hidden_layer_size, input_layer_size+1).copy()   # 这里使用copy函数，否则下面修改Theta的值，nn_params也会一起修改
    ...
    for i in range(m):
        ...
        Theta2_grad = Theta2_grad + \
            np.dot(np.transpose(delta3[i, :].reshape(
                1, -1)), a2[i, :].reshape(1, -1))
        delta2[i, :] = np.dot(delta3[i, :].reshape(
            1, -1), Theta2_x)*sigmoidGradient(z2[i, :])
        Theta1_grad = Theta1_grad + \
            np.dot(np.transpose(delta2[i, :].reshape(
                1, -1)), a1[i, :].reshape(1, -1))
...
def checkGradient(Lambda=0):
    ...
    num_labels = 3
    m = 5
    initial_Theta1 = debugInitializeWeights(
    ...
    nn_params = np.vstack((initial_Theta1.reshape(-1, 1),
                          initial_Theta2.reshape(-1, 1)))  # 展开theta
    '''BP求出梯度'''
