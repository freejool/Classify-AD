...
dataset = pd.read_csv('code/K-Means/chars.csv')
...
for i in range(2):
    ...
    for j in range(numSamples):
        if estimator.labels_[j] == i and dataset.iloc[j, -1] == 3:
            corrctnum[-1] += 1
        elif estimator.labels_[j] == 1-i and dataset.iloc[j, -1] != 3:
            corrctnum[-1] += 1
